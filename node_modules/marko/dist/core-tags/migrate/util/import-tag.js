...
module.exports = function importTag(importIdentifier, context) {
  ...
  const createTagName = fileName => camelCase(fileName, {
    pascalCase: true
  }).replace(/^[^$A-Z_]|[^0-9A-Z_$]/gi, "_");

  const getIdentifierName = importIdentifier => {
    let fileName;
    if (isImportedAsTag) fileName = importIdentifier.slice(1, -1);else {
  ...
  importTag.tagString = `import ${identifier} from ${JSON.stringify(importIdentifier)}`;
